...
zz.pick = function (a, b) {
    ...
    return zz.shuffle(elementer).slice(a);
}
...
zz.table = function(row,col,obj) {
        ...
        for (i=0; i< row; i++) {
          ...
          for (j=0; j<col; j++) {
              t += '<td>'
        t += (obj.data && obj.data[i] && obj.data[i][j] !== undefined) ? obj.data[i][j] : '';
        t += '</td>';
          }
          t += '</tr>';
        }
...
zz.lagInput = function (id='myid', type='text', alternativer = {}) {
  ...
  switch (type) {
    ...
    case "select":
      ...
      t = '<label for="' + id + '">' + ledetekst 
      + zz.makeSelect(id, options, choice) + '</label>';
      break;
    case "button":
